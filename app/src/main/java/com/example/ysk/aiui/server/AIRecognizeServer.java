package com.example.ysk.aiui.server;
...
import com.example.ysk.aiui.util.FucUtil;
import com.example.ysk.aiui.util.PatternUtil;
...
public class AIRecognizeServer extends Service {
    ...
    /**
     * 检查代理是否创建成功，首先构造AIUI代理。(AIUI功能模块是在原语音识别基础上构造起来的，当识别语音时，返回识别结果，同时也返回讯飞的AI语义匹配结果)
     */
    ...
    private class AIUIHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case WAIT_FOR_STOP:
                    removeMessages(STOP_VOICE_NLP);                                         // 移除队列总延迟的消息
            ...
            switch (event.eventType) {
                ...
                case AIUIConstant.EVENT_RESULT: {
                    ...
                    try {
                        ...
                        Log.e("tag", "【AIRecognizeServer】类的方法：【onEvent处理前的内容】: " + "这是处理前内容：" + info);
                        ...
                        if (content.has("cnt_id")) {
                            ...
                            //获取键值
                            Iterator<String> keys = cntJson.keys();
                            while (keys.hasNext()){
                                String key = keys.next();
                                String value = cntJson.getString(key);
                                System.out.println("key: "+key+",value:"+value);
                            }
                            Log.e("tag", "【AIUIResultListener】类的方法：【onEvent返回的JSON数据】: " + cntJson.toString());
                            String sub = params.optString("sub");
                            if ("nlp".equals(sub)) {
                                // 解析得到语义结果
                                String resultStr = cntJson.optString("intent");
                                Log.e("tag", "【AIUIResultListener】类的方法：【onEvent解析得到的语义】解析得到语义结果: " + resultStr);
